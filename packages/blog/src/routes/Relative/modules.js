...
export default function articleReducer(state = initialState, action) {
    switch (action.type) {
        ...
        case Relative.ARTICLE_COUNT:
            return state.set(Relative.ARTICLE_COUNT, action.count);
    ...
    return dispatch => {
        request.relativeArticles({tag, page, size}).then(res => {
            if (res.code === Constants.SUCCESS_CODE) {
                dispatch(relativeArticleData(res.data.data));
                dispatch(articleTag(tag));
                dispatch(articleCount(res.data.count));
            } else if (res.code === 3) {
